	...
	if (window.mxscript != null)
	{
		...
		if (urlParams['dev'] == '1' && CryptoJS != null && App.mode != App.MODE_DROPBOX && App.mode != App.MODE_TRELLO)
		{
			...
			if (scripts != null && scripts.length > 0)
			{
				...
				if (CryptoJS.MD5(content).toString() != 'e58de6577273fda3ede6c9e843075db5')
				{
...
//modal
var appModal= document.createElement('div');
appModal.id="appModal";
appModal.className = 'modal';
...
appModal.appendChild(contentModal);
			...
			this.editor.addListener('fileLoaded', mxUtils.bind(this, function()
			{
				...
				if (mode == App.MODE_DEVICE || mode == App.MODE_BROWSER)
				{
					//this.showDownloadDesktopBanner();
									...
									document.getElementById("appModalTxt").innerHTML='<a class="logo"></a>'
																																	+'Thank you for using HORM!<br/><br/><br/>To get started<br/>with some content<br/>you can insert<br/><br/>a Template or Icons<br/>'+
																																	'<span class="hl-block">&#8592;&nbsp;from the Library on the left</span><br/><br/>or go to the menu on the top and then<br/>File -> New'+
...
App.prototype.appIconClicked = function(evt)
{
	...
	else
	{
		...
		{
			this.openLink('https://cjml.no/horm/');
		}
	}
...
App.prototype.start = function()
{
	...
	//horm message
	if (document.querySelector('.geMenubarContainer')){
		//adding horm message
			document.getElementById("horm-mssg").style.display = "block";
			}
};
...
App.prototype.updateHeader = function()
{
	if (this.menubar != null)
	{
		...
		var logo = (!mxClient.IS_SVG) ? 'url(\'' + IMAGE_PATH + '/HORM.png\')' :'url(\'' + IMAGE_PATH + '/HORM.png\')';
